...
def main():
    ...
    for time, clients in stats[:100]:  # Affichage des 100 premiers résultats
        type_requete = attribuer_type_requete(
            C)  # Affectation d'un type de requête
        groupe_index = type_requete  # Le groupe correspond directement au type de requête
        # Sélection d'un serveur dans le groupe
        serveur_index = groupe_index * K + \
            (clients - 1) % K if clients > 0 else None

        if serveur_index is not None:
            # Mise à jour du serveur recevant la requête
            SERVEURS[serveur_index] += 1

        print(f"\nTemps: {time:.2f} \nNombre de clients dans la file ou en service: {clients} "
              f"\nGroupe de serveurs: {groupe_index}, "
              f"Serveur attribué: {
                  serveur_index if serveur_index is not None else 'Aucun'}, "
